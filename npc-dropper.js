(() => {
  const pageActive = () => document.body.classList.contains('page-npc-dropper');
  ...
  const messages = {
    ...
    fr: {
      ...
      behaviorIdle: 'Idle',
      behaviorPatrol: 'Patrouille',
      behaviorWander: 'Errance',
      collision: 'Collision',
  ...
  const boot = () => {
    ...
    const itemCacheKey = '8bits-npc-cache-state';
    ...
    const normalizeItemAsset = (asset, fallbackNumber = 1) => {
      ...
      return {
        ...
        behavior: ['idle', 'patrol', 'wander'].includes(asset?.behavior) ? asset.behavior : 'idle',
    ...
    const renderAssetList = () => {
      ...
      visibleAssets.forEach((asset) => {
        ...
        const npcNameField = document.createElement('div');
        npcNameField.className = 'asset-field';
        const npcNameLabel = document.createElement('label');
        npcNameLabel.className = 'panel-label';
        npcNameLabel.textContent = msg('npcName', 'NPC name');
        const npcNameInput = document.createElement('input');
        npcNameInput.className = 'asset-input';
        npcNameInput.type = 'text';
        npcNameInput.value = asset.npcName || '';
        npcNameField.appendChild(npcNameLabel);
        npcNameField.appendChild(npcNameInput);
        ...
        ['down', 'up', 'left', 'right'].forEach((value) => {
          ...
          option.value = value;
          option.textContent = msg(`facing${value[0].toUpperCase()}${value.slice(1)}`, value);
          facingSelect.appendChild(option);
        });
        facingSelect.value = asset.facing || 'down';
        facingField.appendChild(facingLabel);
        facingField.appendChild(facingSelect);

        const behaviorField = document.createElement('div');
        behaviorField.className = 'asset-field';
        const behaviorLabel = document.createElement('label');
        behaviorLabel.className = 'panel-label';
        behaviorLabel.textContent = msg('behavior', 'Behavior');
        const behaviorSelect = document.createElement('select');
        behaviorSelect.className = 'asset-select';
        ['idle', 'patrol', 'wander'].forEach((value) => {
          const option = document.createElement('option');
          option.value = value;
          option.textContent = msg(`behavior${value[0].toUpperCase()}${value.slice(1)}`, value);
          behaviorSelect.appendChild(option);
        });
        behaviorSelect.value = asset.behavior || 'idle';
        ...
        row.appendChild(npcNameField);
        row.appendChild(interactionField);
        row.appendChild(facingField);
        row.appendChild(behaviorField);
        ...
        npcNameInput.addEventListener('input', () => {
          asset.npcName = String(npcNameInput.value || '').trim();
          scheduleSave();
        });
        ...
        facingSelect.addEventListener('change', () => {
          asset.facing = ['down', 'up', 'left', 'right'].includes(facingSelect.value) ? facingSelect.value : 'down';
          scheduleSave();
        });
        behaviorSelect.addEventListener('change', () => {
          asset.behavior = ['idle', 'patrol', 'wander'].includes(behaviorSelect.value) ? behaviorSelect.value : 'idle';
          scheduleSave();
        });
    ...
    const renderMapCell = (cell, index) => {
      ...
      if (view === 'items') {
        if (npcLayer?.imageUrl) {
          cell.style.backgroundImage = `url(${npcLayer.imageUrl})`;
          cell.style.backgroundSize = npcLayer.size;
          cell.style.backgroundPosition = npcLayer.position;
    ...
    const buildExportPayload = () => {
      ...
      return window.EightBitsMapSchema.compactPayload({
        ...
        npcAssets: state.assets.map((asset) => ({
          ...
          npcName: asset.npcName || '',
          interactionId: asset.interactionId || '',
          facing: asset.facing || 'down',
          behavior: asset.behavior || 'idle',
          number: asset.number,
          color: asset.color,
          cacheKey: asset.cacheKey,
          cols: asset.cols,
    ...
    const hasUsableBasePayload = (payload) => {
      ...
      const hasNpcs = (Array.isArray(payload?.npcAssets) && payload.npcAssets.length > 0)
        || (Array.isArray(payload?.npcLayer?.cells) && payload.npcLayer.cells.some(Boolean));
      return Boolean(mapId || mapName || hasBaseAssets || hasBaseCells || hasMarkers || hasProps || hasItems || hasNpcs);
    };
    ...
    const loadProjectState = async () => {
      const activeDoc = projectManager?.getStageActiveDocument('npcDropper');
      const activePayload = activeDoc?.payload || projectManager?.getStageActiveCache('npcDropper') || null;
      const latestProjectMap = projectManager?.getLatestUnifiedMap?.([currentStageName, upstreamStageName, 'propDropper', 'mapCreator']);
      const doc = hasUsableBasePayload(activePayload)
        ...
        : latestProjectMap;
      const payload = doc?.payload || projectManager?.getUpstreamPayload('npcDropper');
      if (!hasUsableBasePayload(payload)) return false;
